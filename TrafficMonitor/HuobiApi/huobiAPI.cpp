...
#include <atlbase.h>//W2A used
#include <atlconv.h>//W2A used
...
{
	//GetOfPost = "GET"/"POST";
	CString Url(_T("api.huobipro.com"));
	...
	CString str1;
	for (int i = 0; i < 32; ++i) {
		str.Format(_T("%02lx"), (unsigned long)hmac.digest[i]);
		str1 += str;
	}
	CString  sendmsg(_T("https://"));
	sendmsg = sendmsg + Url + Sub + _T("?");//地址+方法+?
...
{
	USES_CONVERSION;
	string str(W2A(inCStr.GetBuffer(0)));
...
{
	ofstream outfile;
	outfile.open(_T("err_log.txt"), ios::app);
	if (!outfile) //检查文件是否正常打开//不是用于检查文件是否存在
	{
		MessageBox(NULL, msg, _T("err_log.txt打开失败！"), MB_OK);
		abort(); //打开失败，结束程序
	}
	else
	{
		outfile << Get_Times() << _T("\t") << _T("huobiAPI::") << fxname << _T("\t") << msg << endl;
		outfile.close();
	}
}
...
CString huobiAPI::Get_id()
{
	...
	if (msg != "Can't InternetOpen!"&&msg != "Can't InternetOpenUrl!"&&msg != "Can't InternetReadFile!"&&-1==msg.Find(_T("error")))//发送成功
	{
...
CString huobiAPI::Post_自写路径通用(CString phat, CString *instr, int mun)
{
	...
	CString Q[8] = { "AccessKeyId", "=" + Access_Key,
		...
		"SignatureVersion", "=2",
		"Timestamp","=" + Get_Times() };
	sendmsg = Get_Sendmsg(Q, 8, _T("/v1/order/orders/batchcancel"), _T("POST"));
	sendmsg.Delete(0, sendmsg.Find(_T("/v1"), 0));
	CString senddata = "{\"";
	for (int i = 0; i<mun; i+=2)
	{
		senddata += instr[i];
	...
	CString msg = PostRequest(_T("api.huobipro.com"), sendmsg, "");
	if (-1 == msg.Find(_T("error")))//发送成功
	{
		//msg.Delete(0, msg.Find("data", 0));
		// CString sstr = find_str(msg, "[0-9]+");
		return msg;
...
CString huobiAPI::GetMarketDetailMerged(CString phat, CString* instr, int mun)
{
	int j = 8;
	CString Q[64] = { _T("0") };
	CString Q1[8] = { _T("AccessKeyId"), _T("=") + Access_Key,
		_T("SignatureMethod"), _T("=HmacSHA256"),
		_T("SignatureVersion"), _T("=2"),
		_T("Timestamp"), _T("=") + Get_Times() };
	for (int i = 0; i < 8; i++)
	{
		Q[i] = Q1[i];
	}
	...
	CString sendmsg = Get_Sendmsg(Q, mun + 8, phat, _T("GET"));
	CString msg = GetRequest(sendmsg);
	if (msg != _T("Can't InternetOpen!") && msg != _T("Can't InternetOpenUrl!") && msg != _T("Can't InternetReadFile!") && -1 == msg.Find(_T("error")))//发送成功
	{
		wcout << msg.GetString() << endl;
		msg.Delete(0, msg.Find(_T("close"), 0));
		CString sstr = find_str(msg, "[0-9]+[/.]*[0-9]*");
		return sstr;
	}
	CString str;
	str.Format(_T("%d"), mun);
	writeLog(msg, _T("Get_路径无参数通用(phat:") + phat + _T(", *instr:x,mun") + str + _T(")"));
	return msg;
